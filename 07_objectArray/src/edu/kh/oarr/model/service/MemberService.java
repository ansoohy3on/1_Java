...
public class MemberService {
	...
	// memberArr의 비어 있는 인덱스 번호 반환하는 메소드
	// 단, 비어 있는 인덱스가 없으면 -1 반환
	public int emptyIndex() {
		
		// memberArr 배열을 0번 인덱스부터 끝까지 접근해서
		// 참조하는 값이 null인 경우인 인덱스를 반환
		for(int i = 0; i < memberArr.length; i++) {
			if(memberArr[i] == null) {
				return i; // 현재 메소드를 종료하고 호출한 곳으로 i 값을 가지고 돌아감
			}			  // 그렇기 때문에 else문 따로 작성 안 해 줘도 됨
		}
			...
			if(memberArr[i] != null) { // 회원의 정보가 있을 경우
				...
				if(memberArr[i].getMemberId().equals(memberId) &&
				   memberArr[i].getMemberPw().equals(memberPw)) {
					...
					// 3) 로그인 회원 정보 객체(Member)를 참조하는 변수
					//	  현재 접근 중인 memberArr[i] 요소에 저장된 주소를 얕은 복사
					loginMember = memberArr[i];
					break; // 더 이상 같은 아이디, 비밀번호가 없기 때문에
				}
			}
		}
	...
	public String selectMember() {
		...
		// 로그인이 안 되어 있으면 "로그인 후 이용해 주세요." 반환
		if(loginMember == null) {
	...
	public void searchRegion() {
		...
		for(int i = 0; i < memberArr.length; i++){
			...
			// 	  입력받은 지역(inputRegion)과 같을 경우(equals()), 회원의 아이디, 이름을 출력
