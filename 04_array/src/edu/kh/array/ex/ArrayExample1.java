...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		// num이 작성된 자리에 num에 저장된 값을 읽어 와서 출력
		...
		// 2-3. 배열 요소 값 대입
		// arr은 int[] 참조형 변수이지만
		// arr[0]은 int 자료형 변수이기 때문에 정수 값을 대입할 수 있다.
		arr[0] = 10;
		arr[1] = 50;
		arr[2] = 1000;
		...
		System.out.println(arr[0]); // arr이 참조하고 있는 배열의 0번 인덱스 값을 얻어 옴
		System.out.println(arr[1]); // arr이 참조하고 있는 배열의 1번 인덱스 값을 얻어 옴
		System.out.println(arr[2]); // arr이 참조하고 있는 배열의 2번 인덱스 값을 얻어 옴
	...
	public void ex3() {
		...
		// heap 영역에 새로 생성된 double 5칸 짜리 double[]의 시작 주소를 height에 대입
	...
	public void ex4() {
		...
		System.out.printf("입력 받을 인원 수 : ");
		int input = sc.nextInt();
		
		// 배열 선언 및 할당
		// 할당할 배열의 크기는 입력 받은 크기(input)만큼
		...
		for(int i = 0; i < score.length; i++) {
			...
			sum += score[i]; // 점수를 입력받자마자 sum에 누적
	...
	public void ex5() {
		...
		for(int i = 0; i < arr.length; i++) {
			arr[i] = (char)('A' + i); // 'A' 다음 문자에 대응되는 번호
			// A == 65
			// B == 66
			// C == 67
		...
		int[] arr2 = new int[4];
		System.out.println(arr2); // [I@27808f31
		
		// Arrays.toString(배열형) : 모든 요소 값을 출력
	...
	public void ex9() {
		...
		// 2. 문자 하나를 입력받아 일치하는 문자가 char 배열에 맞게 존재하는지 확인
		...
		// 3) String.charAt(index) : 문자열에서 특정 index에 위치한 문자 하나를 얻어 옴
		// 	  ex) "Hello".charAt(1)  -> 'e'
		// 		   01234 (인덱스 번호)
		...
		char ch = sc.nextLine().charAt(0);
				// nextChar()는 없으므로 nextLine()의 첫 번째 문자 얻기
				// String.charAt(0); : 문자열 제일 앞 문자 얻어 오기
